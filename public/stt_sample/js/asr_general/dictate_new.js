(function(window){
	...
	// Defaults
	var SERVER = "wss://puzzle-ai.com:8021/worker/ws/status";
	var SERVER_STATUS = "wss://puzzle-ai.com:8021/worker/ws/status";
	var REFERENCE_HANDLER = "https://puzzle-ai.com/dynamic/reference";
	...
	var RECORDER_WORKER_PATH = 'https://127.0.0.1:8443/amore/asr/js/asr_general/recorderWorker.js';
	...
	var Dictate = function(cfg) {
		...
		// Handles being called several times to update labels. Preserve values.
			...
			for (let i = 0; i !== deviceInfos.length; ++i) {
				...
				if(deviceInfo.label.includes("Webcam")){    // 이부분에 마이크 Label 값을 주면 됩니다. 예) Realtek  //  Webcam  등
					if(deviceInfo.kind === 'audioinput'){
						//alert(deviceInfos[i].label);
						Inputdevice = deviceInfos[i];
						break;
					}
				}

			}
			//alert(Inputdevice.label);
			audioSource = Inputdevice.deviceId;
			constraints = {
				audio: {deviceId: audioSource}
			};

			if (navigator.mediaDevices.getUserMedia(constraints)) {
					navigator.mediaDevices.getUserMedia(constraints).then(startUserMedia).catch(handleError);
		...
		function startUserMedia(stream) {
			...
			//alert(config.recorderWorkerPath);
			config.recorderWorkerPath = "./js/asr_general/recorderWorker.js";
